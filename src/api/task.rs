...
impl ResponseError for TaskError {
  ...
  fn status_code(&self) -> StatusCode {
    match self {
      ...
      TaskError::TaskUpdateFail -> StatusCode::FAILED_DEPENDENCY,
...
#[get("/task/{task_global_id}")] // Returns Struct or Error
...
#[put("/task/{task_global_id}/start")] // Returns Struct or Error
pub async fn start_task(
  ddb_repo: Data<DDBRepository>,
  tast_identifier: Path<TaskIdentifier>
) -> Result<Json<TaskIdentifier>, TaskError>  {
  state_transtion(
    ddb_repo,
    task_identifier.into_inner().task_global_id,
